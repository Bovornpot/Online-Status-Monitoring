...
function openFullChartModal(chartType) {
    ...
    // เรียกฟังก์ชันเพื่ออัปเดตกราฟใน modal
    //updateFullChart(chartType, modalSortSelect.value);

    // รอให้ Modal แสดงผลสมบูรณ์ แล้วค่อยเรียก updateFullChart
    requestAnimationFrame(() => {
        let dataMap = chartType === 'region' ? getCountsByField(allBranches, 'region') : getCountsByField(allBranches, 'province');
...
function updateFullChart(chartType, sortDirection) {
    ...
    if (chartType === 'region') updateRegionChart(true, s);
    else updateProvinceChart(true, s);
}