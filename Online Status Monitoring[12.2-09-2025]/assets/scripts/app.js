...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (DOMElements.fullChartSortSelect) {
        DOMElements.fullChartSortSelect.addEventListener('change', () => {
            ...
            // small defer to ensure layout
            setTimeout(() => { try { if (fullChart) fullChart.resize(); } catch(e){} }, 60);
        });
    }
    ...
    // // --- เพิ่มปุ่ม View All ถ้ายังไม่มี listener (ปลอดภัยถ้ามีอยู่แล้ว) ---
    // if (DOMElements.viewAllRegionBtn) {
    //     DOMElements.viewAllRegionBtn.addEventListener('click', () => openFullChartModal('region'));
    // }
    // if (DOMElements.viewAllProvinceBtn) {
    //     DOMElements.viewAllProvinceBtn.addEventListener('click', () => openFullChartModal('province'));
    // }

    // // close modal button (ถ้ามี)
    // if (DOMElements.closeFullChartModalBtn) {
