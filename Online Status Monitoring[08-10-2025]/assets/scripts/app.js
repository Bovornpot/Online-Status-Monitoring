...
let recorderChart; // ตัวแปรสำหรับเก็บ instance ของ Chart เครื่องบันทึกภาพ
...
function setupChart() {
                        ...
                        callbacks: {
                            label: function(context) {
                                ...
                                if (typeof context.parsed === 'object') {
                                    // vertical bar → ใช้ y
                                    value = context.parsed.y ?? context.parsed.x ?? 0;
                                } else {
                                    value = context.parsed ?? context.raw ?? 0;
                                }
                                return `${context.dataset.label}: ${value} สาขา`;
                            },
                            afterBody: function (context) {
                                // index ของ data point
                                const dataIndex = context[0].dataIndex;
                                // dataset ตัวแรก (Online + Offline) รวมกัน
                                const datasets = context[0].chart.data.datasets;
                                let total = 0;
                                datasets.forEach(ds => {
                                    total += ds.data[dataIndex] || 0;
                                });
                                return `จากทั้งหมด: ${total} สาขา`;
                            }
                        }
                    }
                },
    ...
    try { if (recorderChart) recorderChart.destroy(); } catch(e){}
    ...
    // create recorderChart (horizontal)
    if (DOMElements.recorderChartCanvas) {
        const ctxP = DOMElements.recorderChartCanvas.getContext('2d');
        recorderChart = new Chart(ctxP, baseConfig('y', 'กราฟการเชื่อมต่อแยกตามเครื่องบันทึกภาพ (Top 5)'));
        window.recorderChart = recorderChart;
    }
    ...
    window.updateRecorderChart = updateRecorderChart;
    window.openFullChartModal = openFullChartModal;
    window.closeFullChartModal = closeFullChartModal;
}

// --- Helper: สร้าง counts จาก source โดยใช้ field เช่น 'status'/'region'/'province'/'recorder' ---
...
// --- อัปเดตกราฟทั้งหมด (status vertical + region/province/recorder top5 horizontal) ---
function updateCharts() {
    ...
    // 4) Recorder chart (Top5)
    const recorderSort = DOMElements.recorderChartSortSelect ? DOMElements.recorderChartSortSelect.value : 'online_desc';
    updateRecorderChart(false, recorderSort);
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                let value;
                                if (typeof context.parsed === 'object') {
                                    // ถ้าเป็น horizontal bar ใช้ x, ถ้า vertical ใช้ y
                                    value = context.parsed.x ?? context.parsed.y ?? 0;
                                } else {
                                    value = context.parsed ?? context.raw ?? 0;
                                }
                                return `${context.dataset.label}: ${value} สาขา`;
...
function updateProvinceChart(isFullView = false, sortValue = 'online_desc') {
    ...
    if (!isFullView) {
        ...
        // ทำลายกราฟเดิมก่อน เพื่อป้องกันปัญหาการวาดซ้อน
        if (provinceChart) {
            provinceChart.destroy();
        }

        //  สร้างกราฟใหม่ขึ้นมาเสมอ
...
// --- updateRecorderChart: เหมือน region แต่ใช้ field 'recorder' ---
function updateRecorderChart(isFullView = false, sortValue = 'online_desc') {
    const source = (filteredBranches && filteredBranches.length) ? filteredBranches : allBranches;
    recorderMap = getCountsByField(source, 'recorder');
    const top = sortAndSliceCounts(recorderMap, sortValue, 5);
    ...
    if (!isFullView) {
        ...
        // ทำลายกราฟเดิมก่อน เพื่อป้องกันปัญหาการวาดซ้อน
        if (recorderChart) {
            recorderChart.destroy();
        }

        //  สร้างกราฟใหม่ขึ้นมาเสมอ
        recorderChart = new Chart(DOMElements.recorderChartCanvas, {
            type: 'bar',
            data: {
                labels: labels,
                datasets: [
            ...
            options: {
                ...
                maintainAspectRatio: false,
                plugins: {
                    title: { display: true, text: 'กราฟการเชื่อมต่อแยกตามเครื่องบันทึกภาพ (Top 5)', font: { size: 14, weight: 'bold' } },
                    legend: { display: true, position: 'top' },
                    tooltip: {
                        callbacks: {
                            label: function(context) {
        ...
        if (DOMElements.recorderSummaryContainer) {
            // โค้ดสำหรับ update summary cards
            const html = top.map(t => {
                const rate = t.total > 0 ? ((t.online / t.total) * 100).toFixed(1) : 0;
                return `
            ...
            DOMElements.recorderSummaryContainer.innerHTML = html || '<p style="text-align:center;color:#6b7280;">ไม่มีข้อมูล</p>';
        }
        return;
    } else {
        updateFullChart('recorder', sortValue, recorderMap);
    }
}

// --- updateFullChart: ใช้สำหรับ modal view (ทั้ง region/province/recorder) ---
...
function openFullChartModal(chartType = 'region') {
    ...
    // 1. อัปเดต Title 
    let titleText = 'สถานะการเชื่อมต่อแยกตาม ';
    if (chartType === 'province') {
        titleText += '"จังหวัด" ทั้งหมด';
    } else if (chartType === 'recorder') { //  สำหรับเครื่องบันทึก
        titleText += '"เครื่องบันทึกภาพ" ทั้งหมด';
    } else {
        titleText += '"ภาค" ทั้งหมด';
    }
    DOMElements.fullChartModalTitle.textContent = titleText;

    // 2. ตั้งค่า dropdown sort ให้ตรงกับ Tab นั้นๆ
    if (DOMElements.fullChartSortSelect) {
        // ใช้ค่าจาก dropdown ของ tab นั้นๆ หรือใช้ค่าเริ่มต้น 'online_desc'
   ...
   // 4. รอ modal render เสร็จแล้วค่อยสร้าง chart (ใช้ setTimeout ตามที่ผู้ใช้ระบุ)
    setTimeout(() => {
        ...
        if (chartType === 'region') {
            ...
            // Default:
            dataMap = getCountsByField(allBranches, 'region');
        }

        // 5) บังคับ resize อีกครั้งให้แน่ใจ
        if (fullChart) {
            fullChart.resize();
            fullChart.update();
        }
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (DOMElements.fullChartSortSelect) {
        DOMElements.fullChartSortSelect.addEventListener('change', () => {
            ...
            // <<< เพิ่มเงื่อนไขสำหรับ 'recorder'
            let map;
            if (chartType === 'region') {
                map = getCountsByField(allBranches, 'region');
            } else if (chartType === 'province') {
                map = getCountsByField(allBranches, 'province');
            } else if (chartType === 'recorder') {
                map = getCountsByField(allBranches, 'recorder');
            } 
