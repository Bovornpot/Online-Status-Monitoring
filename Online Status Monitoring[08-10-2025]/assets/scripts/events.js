...
function setupEventListeners() {
    ...
    DOMElements.brandFilter.addEventListener('change', applyFilters);
    ...
    DOMElements.tabRecorderBtn.addEventListener('click', () => showChartTab('recorder'));
    
    // Event Listeners สำหรับปุ่ม "ดูข้อมูลทั้งหมด" ใน Tab 'ภาค' 'จังหวัด' และ 'เครื่องบันทึกภาพ'
    ...
    DOMElements.viewAllRecorderBtn.addEventListener('click', () => {
        openFullChartModal('recorder');
        // เรียก resize หลังจากเปิด modal openFullChartModal มีการสร้าง instance ของ fullChart
        setTimeout(() => {
            if (window.fullChart) {
                window.fullChart.resize();
    ...
    DOMElements.recorderChartSortSelect.addEventListener('change', (event) => {
        handleChartSort('recorder', event.target.value);
    });
