...
async function openDetailsModal(id) {
    ...
    const fieldLabels = {
        ...
        phone: 'เบอร์โทรร้าน', note: 'หมายเหตุ', fc: 'FC.', phone_fc: 'เบอร์โทร Hybrid FC',
...
// ฟังก์ชันสำหรับเปิด Full Chart Modal
function openFullChartModal(chartType) {
    // อัปเดตการแสดงผลใน Modal ตามประเภทที่เลือก
    let title = '';
    let modalSortSelect = DOMElements.fullChartSortSelect;

    // กำหนดค่าเริ่มต้นของ dropdown ใน modal ให้ตรงกับ dropdown ในหน้าหลัก
    if (chartType === 'region') {
        title = '📊 ข้อมูลภาคทั้งหมด';
        modalSortSelect.value = DOMElements.regionChartSortSelect.value;
    } else if (chartType === 'province') {
        title = '📊 ข้อมูลจังหวัดทั้งหมด';
        modalSortSelect.value = DOMElements.provinceChartSortSelect.value;
    } else if (chartType === 'recorder') {
        title = '📊 ข้อมูลเครื่องบันทึกภาพทั้งหมด';
        // ใช้ค่าจาก dropdown ของ 'recorder'
        modalSortSelect.value = DOMElements.recorderChartSortSelect.value;
    } 

    // เก็บประเภทกราฟไว้ใน dataset เพื่อใช้ในการเรียงลำดับใน modal
    DOMElements.fullChartModal.dataset.chartType = chartType;
    DOMElements.fullChartModalTitle.textContent = title;

    // แสดง modal ก่อน
    DOMElements.fullChartModal.style.display = 'block';

    // รอให้ Modal แสดงผลสมบูรณ์ แล้วค่อยเรียก updateFullChart
    requestAnimationFrame(() => {
        let dataMap;
        if (chartType === 'region') {
            dataMap = getCountsByField(allBranches, 'region');
        } else if (chartType === 'province') {
            dataMap = getCountsByField(allBranches, 'province');
        } else if (chartType === 'recorder') { // <<< เพิ่ม: คำนวณ dataMap สำหรับ 'recorder'
            dataMap = getCountsByField(allBranches, 'recorder');
        } else {
            // Fallback
            dataMap = {};
        }
        
        // เรียกฟังก์ชัน updateFullChart โดยส่ง dataMap ไปด้วย
        updateFullChart(chartType, modalSortSelect.value, dataMap);
    });
}

// ฟังก์ชันสำหรับปิด Full Chart Modal
function closeFullChartModal() {
    DOMElements.fullChartModal.style.display = 'none';
}

// ฟังก์ชันสำหรับอัปเดตกราฟใน Modal
function updateFullChart(chartType, sortDirection) {
    // preferred: call ui.js rendering
    if (chartType === 'region') {
        // แก้ไข: ใช้ sortDirection แทน s และเรียกฟังก์ชันอัปเดตกราฟสำหรับภาค
        updateRegionChart(true, sortDirection); 
    } else if (chartType === 'province') {
        // แก้ไข: ใช้ sortDirection แทน s และเรียกฟังก์ชันอัปเดตกราฟสำหรับจังหวัด
        updateProvinceChart(true, sortDirection);
    } else if (chartType === 'recorder') { // <<< เพิ่ม: logic สำหรับ 'recorder'
        // สมมติว่ามีฟังก์ชันสำหรับอัปเดตกราฟเครื่องบันทึกภาพ
        updateRecorderChart(true, sortDirection);
    }

    // หมายเหตุ: โค้ดจริงสำหรับการวาดกราฟใน Modal ควรใช้ dataMap ที่ถูกส่งมาโดยตรง
    // เช่น: updateChart(fullChart, dataMap, sortDirection, DOMElements.fullChartSummaryContainer, 'ชื่อ Field');
}